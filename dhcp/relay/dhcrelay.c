...
main(int argc, char **argv) {
	...
	if (local_family == AF_INET) {
		...
		if (servers == NULL) {
                  ...
                  /* Temporary code to avoid crash when no server specified */

                  in_addr_t ia_temp;
                  struct server_list *sp_temp = NULL;
                  ia_temp = inet_addr("127.0.0.1");

                  sp_temp = ((struct server_list *)
                             dmalloc(sizeof *sp_temp, MDL));
                  sp_temp->next = servers;
                  servers = sp_temp;
                  memcpy(&sp_temp->to.sin_addr, &ia_temp, sizeof ia_temp);

                  /* End of Temporary code to avoid crash when no server specified */
	...
	if (local_family == AF_INET6)
        ...
        /* Temporary code to avoid running in background */
